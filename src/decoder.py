...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, hidden_state, mask=None):
        ...
        x = torch.zeros_like(hidden_state)

        if self.d_model % self.num_attention_heads != 0:
            raise RuntimeError("d_model must be divisible by num_attention_heads")

        for head in self.heads:
            head_out, _ = head.scaled_dot_product_attention(
                hidden_state, hidden_state, hidden_state, mask
            )
            x += head_out
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state = self.self_attention(hidden_state, mask)
        x = hidden_state + x
